...
export function ConnectWalletButton() {
  ...
  const formattedBalance = useMemo(() => {
    if (subAccountBalanceWei === null) {
      return null;
    }
    try {
      const eth = Number(formatEther(subAccountBalanceWei));
      if (Number.isNaN(eth)) {
        return null;
      }
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span>{isConnected ? truncateAddress(subAccount?.address ?? universalAddress ?? '') : 'Base Wallet'}</span>
