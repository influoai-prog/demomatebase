        ...
        <div className="absolute right-0 z-50 mt-2 w-80 max-w-xs rounded-2xl border border-white/10 bg-slate-950/90 p-4 text-sm text-white/80 shadow-[0_25px_70px_-35px_rgba(56,189,248,0.7)] backdrop-blur-xl">
          ...
          <div className="space-y-3 rounded-2xl border border-white/10 bg-slate-900/40 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[11px] uppercase tracking-[0.35em] text-white/50">Owner Wallet</p>
                <p className="mt-1 font-mono text-xs text-white/80">{truncateAddress(displayAddress, 6, 6)}</p>
              </div>
            ...
            {balanceError && (
              <p className="text-[11px] leading-snug text-rose-300/80">{balanceError}</p>
            )}
          </div>

          {subAccountAddress && (
            <div className="mt-3 space-y-2 rounded-2xl border border-sky-400/20 bg-sky-500/10 p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.35em] text-white/50">Sub Account</p>
                  <p className="mt-1 font-mono text-xs text-white/80">{truncateAddress(subAccountAddress, 6, 6)}</p>
                </div>
                <Sparkles className="h-4 w-4 text-sky-200" />
              </div>
              <p className="text-[11px] leading-relaxed text-white/60">
                Auto-spend permissions use this account to settle Perfect Checkout without repeated approvals.
              </p>
              <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.35em] text-white/50">
                <span>Balance</span>
                <span className="font-mono text-white/80">
